...
public class TileEntityArrowCactus extends TileEntity
{
    ...
    public TileEntityArrowCactus()
    {
        shoottime = 0;
        this.Owner = "";
        this.interval = 1;//Interval in which the cactus shoots						(Standard 25)
        ...
        this.arrowspeed = 1.6F;//Defines the Speed of the arrows shot					(Standard 1.6)
    ...
    /**
     * Allows the entity to update its state. Overridden in most subclasses, e.g. the mob spawner uses this to count ticks and creates a new
     * spawn inside its implementation.
     */
    public void updateEntity()
    {
        ...
        {
            if (PL1.username == this.Owner)
            {//dont shoot if its the owner!   			
                return;
            }
            this.shoottime = 0;
            EntitySkeleton SK1 = new EntitySkeleton(this.worldObj);
            SK1.setPosition(this.xCoord, this.yCoord, this.zCoord);
            EntityArrow arr = new EntityArrow(this.worldObj, SK1, PL1, this.arrowspeed, this.arrowscatter);
            arr.setDamage(arr.getDamage() + this.damagebonus);
            arr.setKnockbackStrength(this.knockbacklevel);
            arr.setFire(this.firetime);
            SK1.playSound("random.bow", 1.0F, 1.0F);
            this.worldObj.spawnEntityInWorld(arr);
            SK1.setDead();
        }
    }
