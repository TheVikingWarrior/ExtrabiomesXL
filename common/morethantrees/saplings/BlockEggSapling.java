...
import morethantrees.treegen.WorldGenEggTrees;
...
public class BlockEggSapling extends BlockFlower
{
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:eggSapling");
    }
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
	{
      ...
      WorldGenEggTrees eggtree = new WorldGenEggTrees();

      if (!eggtree.generate(world, random, i, j, k))
      {
	...
	public void generate(World world, Random rand, int i, int j, int k)
    {
		world.setBlock(i, j, k, 0);
		WorldGenEggTrees eggtree = new WorldGenEggTrees();
		if (!eggtree.generate(world, rand, i, j, k))
		{
