...
public class BlockGrandFirLargeSap extends BlockFlower
{
	private Random rand;
	public static final String[] WOOD_TYPES = new String[] {"grandfir"};
    //private Icon[] blockIcon = new Icon[1];
	
	public BlockGrandFirLargeSap(int i)
	{
		super(i);
		float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:GrandFir");
    }
	...
	public void generateTree(World world, int i, int j, int k, Random random, int l)
	{
		...
		WorldGenGrandFirLargeTree grandfirlarge = new WorldGenGrandFirLargeTree();

		if (!grandfirlarge.generate(world, random, i, j, k))
		{
	...
	public void generate(World world, Random rand, int i, int j, int k)
    {
		world.setBlock(i, j, k, 0);
		WorldGenGrandFirLargeTree grandfirlarge = new WorldGenGrandFirLargeTree();
		if (!grandfirlarge.generate(world, rand, i, j, k))
		{
			world.setBlock(i, j, k, this.blockID);
		}
