...
public class BlockJungleSapling extends BlockFlower
{
    private Random               rand;
    public static final String[] WOOD_TYPES = new String[] { "jungle" };
    
    //private Icon[] blockIcon = new Icon[1];
    
    public BlockJungleSapling(int i)
    {
        super(i);
        float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:sapling");
    }
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
    {
        ...
        WorldGenJungleTrees jungletrees = new WorldGenJungleTrees();
        
        if (!jungletrees.generate(world, random, i, j, k))
        {
    ...
    public void generate(World world, Random rand, int i, int j, int k)
    {
        world.setBlock(i, j, k, 0);
        WorldGenJungleTrees jungletrees = new WorldGenJungleTrees();
        if (!jungletrees.generate(world, rand, i, j, k))
        {
            world.setBlock(i, j, k, this.blockID);
        }
