...
import morethantrees.treegen.WorldGenRainTrees;
...
{
    private Random               rand;
    public static final String[] WOOD_TYPES = new String[] { "water" };
    
    public BlockWaterSapling(int i)
    {
        super(i);
        float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:waterSapling");
    }
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
    {
        ...
        WorldGenRainTrees raintrees = new WorldGenRainTrees();
        
        if (!raintrees.generate(world, random, i, j, k))
        {
    ...
    public void generate(World world, Random rand, int i, int j, int k)
    {
        world.setBlock(i, j, k, 0);
        WorldGenRainTrees raintrees = new WorldGenRainTrees();
        if (!raintrees.generate(world, rand, i, j, k))
        {
            world.setBlock(i, j, k, this.blockID);
        }
