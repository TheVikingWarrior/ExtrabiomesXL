...
import morethantrees.treegen.WorldGenSmokinTrees;
...
{
	private Random rand;
	public static final String[] WOOD_TYPES = new String[] {"smokin"};
    
	public BlockSmokinSapling(int i)
	{
		super(i);
		float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:smokinSapling");
    }
	...
	public void generateTree(World world, int i, int j, int k, Random random, int l)
	{
		...
		WorldGenSmokinTrees worldgensmokintrees = new WorldGenSmokinTrees();

		if (!worldgensmokintrees.generate(world, random, i, j, k))
		{
	...
	public void generate(World world, Random rand, int i, int j, int k)
    {
		world.setBlock(i, j, k, 0);
		WorldGenSmokinTrees worldgensmokintrees = new WorldGenSmokinTrees();
		if (!worldgensmokintrees.generate(world, rand, i, j, k))
		{
			world.setBlock(i, j, k, this.blockID);
		}
