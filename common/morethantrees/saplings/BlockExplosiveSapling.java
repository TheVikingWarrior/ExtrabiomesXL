...
import morethantrees.treegen.WorldGenExplosiveTrees;
...
{
	private Random rand;
	public static final String[] WOOD_TYPES = new String[] {"explosive"};
    //private Icon[] blockIcon = new Icon[1];
	
	public BlockExplosiveSapling(int i)
	{
		super(i);
		float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:explosiveSapling");
    }
	...
	public void generateTree(World world, int i, int j, int k, Random random, int l)
	{
		...
		WorldGenExplosiveTrees explosivetrees = new WorldGenExplosiveTrees();

		if (!explosivetrees.generate(world, random, i, j, k))
		{
	...
	public void generate(World world, Random rand, int i, int j, int k)
    {
		world.setBlock(i, j, k, 0);
		WorldGenExplosiveTrees explosivetrees = new WorldGenExplosiveTrees();
		if (!explosivetrees.generate(world, rand, i, j, k))
		{
			world.setBlock(i, j, k, this.blockID);
		}
