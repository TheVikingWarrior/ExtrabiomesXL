...
import morethantrees.treegen.WorldGenWolfTrees;
...
{
	private Random rand;
	public static final String[] WOOD_TYPES = new String[] {"wolf"};
    
	public BlockWolfSapling(int i)
	{
		super(i);
		float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:wolfSapling");
    }
	...
	public void generateTree(World world, int i, int j, int k, Random random, int l)
	{
		...
		WorldGenWolfTrees wolftrees = new WorldGenWolfTrees();

		if (!wolftrees.generate(world, random, i, j, k))
		{
	...
	public void generate(World world, Random rand, int i, int j, int k)
    {
		world.setBlock(i, j, k, 0);
		WorldGenWolfTrees wolftrees = new WorldGenWolfTrees();
		if (!wolftrees.generate(world, rand, i, j, k))
		{
			world.setBlock(i, j, k, this.blockID);
		}
