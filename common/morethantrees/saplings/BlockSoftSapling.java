...
import morethantrees.treegen.WorldGenSoftTrees;
...
{
    private Random               rand;
    public static final String[] WOOD_TYPES = new String[] { "soft" };
    
    public BlockSoftSapling(int i)
    {
        super(i);
        float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:softSapling");
    }
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
    {
        ...
        WorldGenSoftTrees worldgensofttrees = new WorldGenSoftTrees();
        
        if (!worldgensofttrees.generate(world, random, i, j, k))
        {
    ...
    public void generate(World world, Random rand, int i, int j, int k)
    {
        world.setBlock(i, j, k, 0);
        WorldGenSoftTrees worldgensofttrees = new WorldGenSoftTrees();
        if (!worldgensofttrees.generate(world, rand, i, j, k))
        {
            world.setBlock(i, j, k, this.blockID);
        }
