...
import morethantrees.treegen.WorldGenCucumberTree;
...
{
	private Random rand;
	public static final String[] WOOD_TYPES = new String[] {"cucumber"};
    //private Icon[] blockIcon = new Icon[1];
	
	public BlockCucumberSapling(int i)
	{
		super(i);
		float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:cucumberSapling");
    }
	...
	public void generateTree(World world, int i, int j, int k, Random random, int l)
	{
		...
		WorldGenCucumberTree cucumbertree = new WorldGenCucumberTree();
		if (!cucumbertree.generate(world, random, i, j, k))
		{
	...
	public void generate(World world, Random rand, int i, int j, int k)
    {
		world.setBlock(i, j, k, 0);
		WorldGenCucumberTree cucumbertree = new WorldGenCucumberTree();
		if (!cucumbertree.generate(world, rand, i, j, k))
		{
			world.setBlock(i, j, k, this.blockID);
		}	
    }
