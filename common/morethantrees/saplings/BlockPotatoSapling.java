...
import morethantrees.treegen.WorldGenPotatoTree;
import morethantrees.treegen.WorldGenRandomBrownCupShroom;
...
{
    private Random               rand;
    public static final String[] WOOD_TYPES = new String[] { "potato" };
    
    //private Icon[] blockIcon = new Icon[1];
    
    public BlockPotatoSapling(int i)
    {
        super(i);
        float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:potatoSapling");
    }
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
    {
        ...
        WorldGenRandomBrownCupShroom browncupshroom = new WorldGenRandomBrownCupShroom();
        
        if (!browncupshroom.generate(world, random, i, j, k))
        {
    ...
    {
        world.setBlock(i, j, k, 0);
        WorldGenPotatoTree potatoTree = new WorldGenPotatoTree();
        if (!potatoTree.generate(world, rand, i, j, k))
        {
            world.setBlock(i, j, k, this.blockID);
        }
