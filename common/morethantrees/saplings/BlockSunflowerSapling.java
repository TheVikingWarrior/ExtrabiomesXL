...
import morethantrees.treegen.WorldGenSunflowerTrees;
...
{
    private Random               rand;
    public static final String[] WOOD_TYPES = new String[] { "sunflower" };
    
    public BlockSunflowerSapling(int i)
    {
        super(i);
        float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:sunflowerSapling");
    }
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
    {
        ...
        WorldGenSunflowerTrees sunflowertrees = new WorldGenSunflowerTrees();
        
        if (!sunflowertrees.generate(world, random, i, j, k))
        {
    ...
    public void generate(World world, Random rand, int i, int j, int k)
    {
        world.setBlock(i, j, k, 0);
        WorldGenSunflowerTrees sunflowertrees = new WorldGenSunflowerTrees();
        if (!sunflowertrees.generate(world, rand, i, j, k))
        {
            world.setBlock(i, j, k, this.blockID);
        }
