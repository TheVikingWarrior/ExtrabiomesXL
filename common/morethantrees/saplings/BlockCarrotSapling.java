...
{
	private Random rand;
	public static final String[] WOOD_TYPES = new String[] {"carrot"};
    //private Icon[] blockIcon = new Icon[1];
	
	public BlockCarrotSapling(int i)
	{
		super(i);
		float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:carrotSapling");
    }
	...
	public void generateTree(World world, int i, int j, int k, Random random, int l)
	{
		...
		WorldGenCarrotTree carrottree = new WorldGenCarrotTree();

		if (!carrottree.generate(world, random, i, j, k))
		{
	...
	public void generate(World world, Random rand, int i, int j, int k)
    {
		world.setBlock(i, j, k, 0);
		WorldGenCarrotTree carrottree = new WorldGenCarrotTree();
		if (!carrottree.generate(world, rand, i, j, k))
		{
			world.setBlock(i, j, k, this.blockID);
		}
