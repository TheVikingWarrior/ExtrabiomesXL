...
import morethantrees.treegen.WorldGenBalsamFirSmallTree;
...
public class BlockBalsamFirSmallSap extends BlockFlower
{
	...
	public void generateTree(World world, int i, int j, int k, Random random, int l)
	{
		...
		WorldGenBalsamFirSmallTree balsamfirsmall = new WorldGenBalsamFirSmallTree();

		if (!balsamfirsmall.generate(world, random, i, j, k))
		{
	...
	public void generate(World world, Random rand, int i, int j, int k)
    {
		world.setBlock(i, j, k, 0);
		WorldGenBalsamFirSmallTree balsamfirsmall = new WorldGenBalsamFirSmallTree();
		if (!balsamfirsmall.generate(world, rand, i, j, k))
		{
			world.setBlock(i, j, k, this.blockID);
		}
