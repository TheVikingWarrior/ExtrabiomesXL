...
import morethantrees.treegen.WorldGenNorwaySpruceLargeTree;
...
public class BlockNorwaySpruceLargeSap extends BlockFlower
{
    private Random               rand;
    ...
    public BlockNorwaySpruceLargeSap(int i)
    {
        super(i);
        float f = 0.4F;
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
    {
        ...
        WorldGenNorwaySpruceLargeTree norwaysprucelarge = new WorldGenNorwaySpruceLargeTree();
        
        if (!norwaysprucelarge.generate(world, random, i, j, k))
        {
    ...
    public void generate(World world, Random rand, int i, int j, int k)
    {
        world.setBlock(i, j, k, 0);
        WorldGenNorwaySpruceLargeTree norwaysprucelarge = new WorldGenNorwaySpruceLargeTree();
        if (!norwaysprucelarge.generate(world, rand, i, j, k))
        {
            world.setBlock(i, j, k, this.blockID);
        }
