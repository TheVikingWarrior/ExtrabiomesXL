...
import morethantrees.treegen.WorldGenDouglasFirXLargeTree;
...
public class BlockDouglasFirXLargeSap extends BlockFlower
{
    private Random               rand;
    public static final String[] WOOD_TYPES = new String[] { "douglasfir" };
    
    //private Icon[] blockIcon = new Icon[1];
    
    public BlockDouglasFirXLargeSap(int i)
    {
        super(i);
        float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:DouglasFir");
    }
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
    {
        ...
        WorldGenDouglasFirXLargeTree douglasfirxlarge = new WorldGenDouglasFirXLargeTree();
        
        if (!douglasfirxlarge.generate(world, random, i, j, k))
        {
    ...
    public void generate(World world, Random rand, int i, int j, int k)
    {
        world.setBlock(i, j, k, 0);
        WorldGenDouglasFirXLargeTree douglasfirxlarge = new WorldGenDouglasFirXLargeTree();
        if (!douglasfirxlarge.generate(world, rand, i, j, k))
        {
            world.setBlock(i, j, k, this.blockID);
        }
