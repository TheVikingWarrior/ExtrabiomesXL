...
import morethantrees.treegen.WorldGenScotsPineLargeTree;
...
public class BlockScotsPineLargeSap extends BlockFlower
{
    private Random               rand;
    public static final String[] WOOD_TYPES = new String[] { "scotspine" };
    
    public BlockScotsPineLargeSap(int i)
    {
        super(i);
        float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:ScottsPine");
    }
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
    {
        ...
        WorldGenScotsPineLargeTree scotspinelarge = new WorldGenScotsPineLargeTree();
        
        if (!scotspinelarge.generate(world, random, i, j, k))
        {
    ...
    public void generate(World world, Random rand, int i, int j, int k)
    {
        world.setBlock(i, j, k, 0);
        WorldGenScotsPineLargeTree scotspinelarge = new WorldGenScotsPineLargeTree();
        if (!scotspinelarge.generate(world, rand, i, j, k))
        {
            world.setBlock(i, j, k, this.blockID);
        }
