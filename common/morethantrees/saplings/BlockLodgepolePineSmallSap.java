...
import morethantrees.treegen.WorldGenLodgepolePineSmallTree;
...
public class BlockLodgepolePineSmallSap extends BlockFlower
{
	private Random rand;
	public static final String[] WOOD_TYPES = new String[] {"lodgepolepine"};
    //private Icon[] blockIcon = new Icon[1];
	
	public BlockLodgepolePineSmallSap(int i)
	{
		super(i);
		float f = 0.4F;
	...
	public void generateTree(World world, int i, int j, int k, Random random, int l)
	{
		...
		WorldGenLodgepolePineSmallTree lodgepolepinesmall = new WorldGenLodgepolePineSmallTree();

		if (!lodgepolepinesmall.generate(world, random, i, j, k))
		{
	...
	public void generate(World world, Random rand, int i, int j, int k)
    {
		world.setBlock(i, j, k, 0);
		WorldGenLodgepolePineSmallTree lodgepolepinesmall = new WorldGenLodgepolePineSmallTree();
		if (!lodgepolepinesmall.generate(world, rand, i, j, k))
		{
			world.setBlock(i, j, k, this.blockID);
		}
