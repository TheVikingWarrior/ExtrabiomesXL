...
import morethantrees.treegen.WorldGenJATree;
...
{
	private Random rand;
	public static final String[] WOOD_TYPES = new String[] {"apricot"};
    //private Icon[] blockIcon = new Icon[1];
	
	public BlockjapaneseApricotSapling(int i)
	{
		super(i);
		float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:apricot_Sapling");
    }
	...
	public void generateTree(World world, int i, int j, int k, Random random, int l)
	{
		...
		WorldGenJATree jat = new WorldGenJATree();

		if (!jat.generate(world, random, i, j, k))
		{
	...
	public void generate(World world, Random rand, int i, int j, int k)
    {
		world.setBlock(i, j, k, 0);
		WorldGenJATree jat = new WorldGenJATree();
		if (!jat.generate(world, rand, i, j, k))
		{
			world.setBlock(i, j, k, this.blockID);
		}
