...
import morethantrees.treegen.WorldGenSquashTree;
...
{
    private Random               rand;
    public static final String[] WOOD_TYPES = new String[] { "squash" };
    
    public BlockSquashSapling(int i)
    {
        super(i);
        float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:squashSapling");
    }
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
    {
        ...
        WorldGenSquashTree squashtrees = new WorldGenSquashTree();
        
        if (!squashtrees.generate(world, random, i, j, k))
        {
    ...
    public void generate(World world, Random rand, int i, int j, int k)
    {
        world.setBlock(i, j, k, 0);
        WorldGenSquashTree squashtrees = new WorldGenSquashTree();
        if (!squashtrees.generate(world, rand, i, j, k))
        {
            world.setBlock(i, j, k, this.blockID);
        }
