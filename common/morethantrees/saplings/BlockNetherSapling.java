...
import morethantrees.treegen.WorldGenNetherTrees;
...
{
	private Random rand;
	public static final String[] WOOD_TYPES = new String[] {"nether"};
    
	public BlockNetherSapling(int i)
	{
		super(i);
		float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:netherSapling");
    }
	...
	public void generateTree(World world, int i, int j, int k, Random random, int l)
	{
		...
		WorldGenNetherTrees nethertrees = new WorldGenNetherTrees();

		if (!nethertrees.generate(world, random, i, j, k))
		{
	...
	public void generate(World world, Random rand, int i, int j, int k)
    {
		world.setBlock(i, j, k, 0);
		WorldGenNetherTrees nethertrees = new WorldGenNetherTrees();
		if (!nethertrees.generate(world, rand, i, j, k))
		{
			world.setBlock(i, j, k, this.blockID);
		}
