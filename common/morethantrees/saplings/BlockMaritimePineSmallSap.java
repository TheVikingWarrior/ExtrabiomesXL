...
import morethantrees.treegen.WorldGenMaritimePineSmallTree;
...
public class BlockMaritimePineSmallSap extends BlockFlower
{
    private Random               rand;
    public static final String[] WOOD_TYPES = new String[] { "maritimepine" };
    
    public BlockMaritimePineSmallSap(int i)
    {
        super(i);
        float f = 0.4F;
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
    {
        ...
        WorldGenMaritimePineSmallTree maritimesmall = new WorldGenMaritimePineSmallTree();
        
        if (!maritimesmall.generate(world, random, i, j, k))
        {
    ...
    public void generate(World world, Random rand, int i, int j, int k)
    {
        world.setBlock(i, j, k, 0);
        WorldGenMaritimePineSmallTree maritimesmall = new WorldGenMaritimePineSmallTree();
        if (!maritimesmall.generate(world, rand, i, j, k))
        {
            world.setBlock(i, j, k, this.blockID);
        }
