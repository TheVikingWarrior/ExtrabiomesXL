...
import morethantrees.treegen.WorldGenDouglasFirSmallTree;
...
public class BlockDouglasFirSmallSap extends BlockFlower
{
    private Random               rand;
    ...
    public BlockDouglasFirSmallSap(int i)
    {
        super(i);
        float f = 0.4F;
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
    {
        ...
        WorldGenDouglasFirSmallTree douglasfirsmall = new WorldGenDouglasFirSmallTree();
        
        if (!douglasfirsmall.generate(world, random, i, j, k))
        {
    ...
    public void generate(World world, Random rand, int i, int j, int k)
    {
        world.setBlock(i, j, k, 0);
        WorldGenDouglasFirSmallTree douglasfirsmall = new WorldGenDouglasFirSmallTree();
        if (!douglasfirsmall.generate(world, rand, i, j, k))
        {
            world.setBlock(i, j, k, this.blockID);
        }
