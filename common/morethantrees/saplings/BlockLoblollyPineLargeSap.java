...
import morethantrees.treegen.WorldGenLoblollyPineLargeTree;
...
public class BlockLoblollyPineLargeSap extends BlockFlower
{
    private Random               rand;
    public static final String[] WOOD_TYPES = new String[] { "loblollypine" };
    
    //private Icon[] blockIcon = new Icon[1];
    
    public BlockLoblollyPineLargeSap(int i)
    {
        super(i);
        float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:LoblollyPine");
    }
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
    {
        ...
        WorldGenLoblollyPineLargeTree loblollypinelarge = new WorldGenLoblollyPineLargeTree();
        
        if (!loblollypinelarge.generate(world, random, i, j, k))
        {
    ...
    public void generate(World world, Random rand, int i, int j, int k)
    {
        world.setBlock(i, j, k, 0);
        WorldGenLoblollyPineLargeTree loblollypinelarge = new WorldGenLoblollyPineLargeTree();
        if (!loblollypinelarge.generate(world, rand, i, j, k))
        {
            world.setBlock(i, j, k, this.blockID);
        }
