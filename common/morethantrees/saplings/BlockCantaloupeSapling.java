...
import morethantrees.treegen.WorldGenCantaloupeTree;
...
{
	private Random rand;
	public static final String[] WOOD_TYPES = new String[] {"cantaloupe"};
    //private Icon[] blockIcon = new Icon[1];
	
	public BlockCantaloupeSapling(int i)
	{
		super(i);
		float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:cantaloupeSapling");
    }
	...
	public void generateTree(World world, int i, int j, int k, Random random, int l)
	{
		...
		WorldGenCantaloupeTree cantaloupetree = new WorldGenCantaloupeTree();

		if (!cantaloupetree.generate(world, random, i, j, k))
		{
	...
	public void generate(World world, Random rand, int i, int j, int k)
    {
		world.setBlock(i, j, k, 0);
		WorldGenCantaloupeTree cantaloupetree = new WorldGenCantaloupeTree();
		if (!cantaloupetree.generate(world, rand, i, j, k))
		{
			world.setBlock(i, j, k, this.blockID);
		}
