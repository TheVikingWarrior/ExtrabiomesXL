...
import morethantrees.treegen.WorldGenWalnutTrees;
...
{
    private Random               rand;
    public static final String[] WOOD_TYPES = new String[] { "walnut" };
    
    public BlockWalnutSapling(int i)
    {
        super(i);
        float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:walnutSapling");
    }
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
    {
        ...
        WorldGenWalnutTrees walnuttrees = new WorldGenWalnutTrees();
        
        if (!walnuttrees.generate(world, random, i, j, k))
        {
    ...
    public void generate(World world, Random rand, int i, int j, int k)
    {
        world.setBlock(i, j, k, 0);
        WorldGenWalnutTrees walnuttrees = new WorldGenWalnutTrees();
        if (!walnuttrees.generate(world, rand, i, j, k))
        {
            world.setBlock(i, j, k, this.blockID);
        }
