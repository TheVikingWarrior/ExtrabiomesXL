...
import morethantrees.treegen.WorldGenNeverTrees;
...
{
    private Random               rand;
    public static final String[] WOOD_TYPES = new String[] { "never" };
    
    //private Icon[] blockIcon = new Icon[1];
    
    public BlockNeverSapling(int i)
    {
        super(i);
        float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:neverSapling");
    }
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
    {
        ...
        WorldGenNeverTrees nevertrees = new WorldGenNeverTrees();
        
        if (!nevertrees.generate(world, random, i, j, k))
        {
    ...
    public void generate(World world, Random rand, int i, int j, int k)
    {
        world.setBlock(i, j, k, 0);
        WorldGenNeverTrees nevertrees = new WorldGenNeverTrees();
        if (!nevertrees.generate(world, rand, i, j, k))
        {
            world.setBlock(i, j, k, this.blockID);
        }
