...
import morethantrees.treegen.WorldGenGrandFirSmallTree;
...
public class BlockGrandFirSmallSap extends BlockFlower
{
    private Random               rand;
    ...
    public BlockGrandFirSmallSap(int i)
    {
        super(i);
        float f = 0.4F;
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
    {
        ...
        WorldGenGrandFirSmallTree grandfirsmall = new WorldGenGrandFirSmallTree();
        
        if (!grandfirsmall.generate(world, random, i, j, k))
        {
    ...
    public void generate(World world, Random rand, int i, int j, int k)
    {
        world.setBlock(i, j, k, 0);
        WorldGenGrandFirSmallTree grandfirsmall = new WorldGenGrandFirSmallTree();
        if (!grandfirsmall.generate(world, rand, i, j, k))
        {
            world.setBlock(i, j, k, this.blockID);
        }
