...
import morethantrees.treegen.WorldGenNorwaySpruceSmallTree;
...
public class BlockNorwaySpruceSmallSap extends BlockFlower
{
    private Random               rand;
    public static final String[] WOOD_TYPES = new String[] { "norwayspruce" };
    
    //private Icon[] blockIcon = new Icon[1];
    
    public BlockNorwaySpruceSmallSap(int i)
    {
        super(i);
        float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:NorwaySpruce");
    }
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
    {
        ...
        WorldGenNorwaySpruceSmallTree norwaysprucesmall = new WorldGenNorwaySpruceSmallTree();
        
        if (!norwaysprucesmall.generate(world, random, i, j, k))
        {
    ...
    public void generate(World world, Random rand, int i, int j, int k)
    {
        world.setBlock(i, j, k, 0);
        WorldGenNorwaySpruceSmallTree norwaysprucesmall = new WorldGenNorwaySpruceSmallTree();
        if (!norwaysprucesmall.generate(world, rand, i, j, k))
        {
            world.setBlock(i, j, k, this.blockID);
        }
