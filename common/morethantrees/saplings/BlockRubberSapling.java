...
public class BlockRubberSapling extends BlockFlower
{
	...
	public BlockRubberSapling(int i)
	{
		super(i);
		float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:rubberSapling");
    }
	...
	public void generateTree(World world, int i, int j, int k, Random random, int l)
	{
		...
		WorldGenRubberTrees rubbertrees = new WorldGenRubberTrees();

		if (!rubbertrees.generate(world, random, i, j, k))
		{
	...
	public void generate(World world, Random rand, int i, int j, int k)
    {
		world.setBlock(i, j, k, 0);
		WorldGenRubberTrees rubbertrees = new WorldGenRubberTrees();
		if (!rubbertrees.generate(world, rand, i, j, k))
		{
			world.setBlock(i, j, k, this.blockID);
		}
