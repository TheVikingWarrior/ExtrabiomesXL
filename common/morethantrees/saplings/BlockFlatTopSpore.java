...
import morethantrees.treegen.WorldGenFlatTopShroom;
...
public class BlockFlatTopSpore extends BlockFlower
{
    private Random               rand;
    ...
    public BlockFlatTopSpore(int i)
    {
        super(i);
        float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:flattop");
    }
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
    {
        ...
        WorldGenFlatTopShroom flattopshroom = new WorldGenFlatTopShroom();
        
        if (!flattopshroom.generate(world, random, i, j, k))
        {
    ...
    public void generate(World world, Random rand, int i, int j, int k)
    {
        world.setBlock(i, j, k, 0);
        WorldGenFlatTopShroom flattopshroom = new WorldGenFlatTopShroom();
        if (!flattopshroom.generate(world, rand, i, j, k))
        {
            world.setBlock(i, j, k, this.blockID);
        }
