...
import morethantrees.treegen.WorldGenLifeTrees;
...
{
    private Random               rand;
    public static final String[] WOOD_TYPES = new String[] { "life" };
    
    //private Icon[] blockIcon = new Icon[1];
    
    public BlockLifeSapling(int i)
    {
        super(i);
        float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:lifeSapling");
    }
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
    {
        ...
        WorldGenLifeTrees lifetrees = new WorldGenLifeTrees();
        
        if (!lifetrees.generate(world, random, i, j, k))
        {
    ...
    public void generate(World world, Random rand, int i, int j, int k)
    {
        world.setBlock(i, j, k, 0);
        WorldGenLifeTrees lifetrees = new WorldGenLifeTrees();
        if (!lifetrees.generate(world, rand, i, j, k))
        {
            world.setBlock(i, j, k, this.blockID);
        }
