...
import morethantrees.treegen.WorldGenSmiteTrees;
...
{
	private Random rand;
	public static final String[] WOOD_TYPES = new String[] {"smite"};
    
	public BlockSmiteSapling(int i)
	{
		super(i);
		float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:smiteSapling");
    }
	...
	public void generateTree(World world, int i, int j, int k, Random random, int l)
	{
		...
		WorldGenSmiteTrees worldgensmitetrees = new WorldGenSmiteTrees();

		if (!worldgensmitetrees.generate(world, random, i, j, k))
		{
	...
	public void generate(World world, Random rand, int i, int j, int k)
    {
		world.setBlock(i, j, k, 0);
		WorldGenSmiteTrees worldgensmitetrees = new WorldGenSmiteTrees();
		if (!worldgensmitetrees.generate(world, rand, i, j, k))
		{
			world.setBlock(i, j, k, this.blockID);
		}
