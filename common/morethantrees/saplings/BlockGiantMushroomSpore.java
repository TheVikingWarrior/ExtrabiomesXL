...
import morethantrees.treegen.WorldGenGiantMushroomShroom;
...
public class BlockGiantMushroomSpore extends BlockFlower
{
    private Random               rand;
    public static final String[] WOOD_TYPES = new String[] { "giantmushroom" };
    
    //private Icon[] blockIcon = new Icon[1];
    
    public BlockGiantMushroomSpore(int i)
    {
        super(i);
        float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:giantmushroom");
    }
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
    {
        ...
        WorldGenGiantMushroomShroom giantshroom = new WorldGenGiantMushroomShroom();
        
        if (!giantshroom.generate(world, random, i, j, k))
        {
    ...
    public void generate(World world, Random rand, int i, int j, int k)
    {
        world.setBlock(i, j, k, 0);
        WorldGenGiantMushroomShroom giantshroom = new WorldGenGiantMushroomShroom();
        if (!giantshroom.generate(world, rand, i, j, k))
        {
            world.setBlock(i, j, k, this.blockID);
        }
