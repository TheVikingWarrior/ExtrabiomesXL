...
import morethantrees.treegen.WorldGenMaritimePineLargeTree;
...
public class BlockMaritimePineLargeSap extends BlockFlower
{
    private Random               rand;
    public static final String[] WOOD_TYPES = new String[] { "maritimepine" };
    
    //private Icon[] blockIcon = new Icon[1];
    
    public BlockMaritimePineLargeSap(int i)
    {
        super(i);
        float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:maritimepine");
    }
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
    {
        ...
        WorldGenMaritimePineLargeTree maritimelarge = new WorldGenMaritimePineLargeTree();
        
        if (!maritimelarge.generate(world, random, i, j, k))
        {
    ...
    public void generate(World world, Random rand, int i, int j, int k)
    {
        world.setBlock(i, j, k, 0);
        WorldGenMaritimePineLargeTree maritimelarge = new WorldGenMaritimePineLargeTree();
        if (!maritimelarge.generate(world, rand, i, j, k))
        {
            world.setBlock(i, j, k, this.blockID);
        }
