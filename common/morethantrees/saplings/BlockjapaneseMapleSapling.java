...
import morethantrees.treegen.WorldGenJMTree;
...
public class BlockjapaneseMapleSapling extends BlockFlower
{
    ...
    public BlockjapaneseMapleSapling(int i)
    {
        super(i);
        float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:mapleSapling");
    }
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
    {
        ...
        WorldGenJMTree jm = new WorldGenJMTree();
        
        if (!jm.generate(world, random, i, j, k))
        {
    ...
    public void generate(World world, Random rand, int i, int j, int k)
    {
        world.setBlock(i, j, k, 0);
        WorldGenJMTree jm = new WorldGenJMTree();
        if (!jm.generate(world, rand, i, j, k))
        {
            world.setBlock(i, j, k, this.blockID);
        }
