...
import morethantrees.treegen.WorldGenJCBTree;
...
{
	private Random rand;
	public static final String[] WOOD_TYPES = new String[] {"cherryBlossom"};
    //private Icon[] blockIcon = new Icon[1];
	
	public BlockjapanesecherryBlossomSapling(int i)
	{
		super(i);
		float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:cherry_Sapling");
    }
	...
	public void generateTree(World world, int i, int j, int k, Random random, int l)
	{
		...
		WorldGenJCBTree jcb = new WorldGenJCBTree();

		if (!jcb.generate(world, random, i, j, k))
		{
	...
	public void generate(World world, Random rand, int i, int j, int k)
    {
		world.setBlock(i, j, k, 0);
		WorldGenJCBTree jcb = new WorldGenJCBTree();
		if (!jcb.generate(world, rand, i, j, k))
		{
			world.setBlock(i, j, k, this.blockID);
		}
