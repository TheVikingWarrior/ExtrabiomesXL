...
import morethantrees.treegen.WorldGenLodgepolePineLargeTree;
...
public class BlockLodgepolePineLargeSap extends BlockFlower
{
	private Random rand;
	public static final String[] WOOD_TYPES = new String[] {"lodgepolepine"};
    //private Icon[] blockIcon = new Icon[1];
	
	public BlockLodgepolePineLargeSap(int i)
	{
		super(i);
		float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:LodgePole");
    }
	...
	public void generateTree(World world, int i, int j, int k, Random random, int l)
	{
		...
		WorldGenLodgepolePineLargeTree lodgepolepinelarge = new WorldGenLodgepolePineLargeTree();

		if (!lodgepolepinelarge.generate(world, random, i, j, k))
		{
	...
	public void generate(World world, Random rand, int i, int j, int k)
    {
		world.setBlock(i, j, k, 0);
		WorldGenLodgepolePineLargeTree lodgepolepinelarge = new WorldGenLodgepolePineLargeTree();
		if (!lodgepolepinelarge.generate(world, rand, i, j, k))
		{
			world.setBlock(i, j, k, this.blockID);
		}
