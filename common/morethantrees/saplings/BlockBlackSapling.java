...
import morethantrees.treegen.WorldGenBlackTrees;
...
{
	private Random rand;
	public static final String[] WOOD_TYPES = new String[] {"blackSapling"};
    //private Icon blockIcon;
	
	public BlockBlackSapling(int i)
	{
		super(i);
		float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:blackSapling");
    }
	...
	public void generateTree(World world, int i, int j, int k, Random random, int l)
	{
		...
		WorldGenBlackTrees black = new WorldGenBlackTrees();

		if (!black.generate(world, random, i, j, k))
		{
	...
	public void generate(World world, Random rand, int i, int j, int k)
    {
		world.setBlock(i, j, k, 0);
		WorldGenBlackTrees black = new WorldGenBlackTrees();
		if (!black.generate(world, rand, i, j, k))
		{
			world.setBlock(i, j, k, this.blockID);
		}
