...
import morethantrees.treegen.WorldGenDouglasFirLargeTree;
...
public class BlockDouglasFirLargeSap extends BlockFlower
{
    private Random               rand;
    ...
    public BlockDouglasFirLargeSap(int i)
    {
        super(i);
        float f = 0.4F;
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
    {
        ...
        WorldGenDouglasFirLargeTree douglasfirlarge = new WorldGenDouglasFirLargeTree();
        
        if (!douglasfirlarge.generate(world, random, i, j, k))
        {
    ...
    public void generate(World world, Random rand, int i, int j, int k)
    {
        world.setBlock(i, j, k, 0);
        WorldGenDouglasFirLargeTree douglasfirlarge = new WorldGenDouglasFirLargeTree();
        if (!douglasfirlarge.generate(world, rand, i, j, k))
        {
            world.setBlock(i, j, k, this.blockID);
        }
