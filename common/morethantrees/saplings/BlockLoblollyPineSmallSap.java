...
import morethantrees.treegen.WorldGenLoblollyPineSmallTree;
...
public class BlockLoblollyPineSmallSap extends BlockFlower
{
	private Random rand;
	public static final String[] WOOD_TYPES = new String[] {"mushroom"};
    //private Icon[] blockIcon = new Icon[1];
	
	public BlockLoblollyPineSmallSap(int i)
	{
		super(i);
		float f = 0.4F;
	...
	public void generateTree(World world, int i, int j, int k, Random random, int l)
	{
		...
		WorldGenLoblollyPineSmallTree loblollypinesmall = new WorldGenLoblollyPineSmallTree();

		if (!loblollypinesmall.generate(world, random, i, j, k))
		{
	...
	public void generate(World world, Random rand, int i, int j, int k)
    {
		world.setBlock(i, j, k, 0);
		WorldGenLoblollyPineSmallTree loblollypinesmall = new WorldGenLoblollyPineSmallTree();
		if (!loblollypinesmall.generate(world, rand, i, j, k))
		{
			world.setBlock(i, j, k, this.blockID);
		}
