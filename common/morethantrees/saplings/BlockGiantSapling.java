...
{
	private Random rand;
	public static final String[] WOOD_TYPES = new String[] {"giant"};
    //private Icon[] blockIcon = new Icon[1];
	
	public BlockGiantSapling(int i)
	{
		super(i);
		float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:giantSapling");
    }
	...
	public void generateTree(World world, int i, int j, int k, Random random, int l)
	{
		...
		WorldGenGiantTrees gianttrees = new WorldGenGiantTrees();

		if (!gianttrees.generate(world, random, i, j, k))
		{
	...
	public void generate(World world, Random rand, int i, int j, int k)
    {
		world.setBlock(i, j, k, 0);
		WorldGenGiantTrees gianttrees = new WorldGenGiantTrees();
		if (!gianttrees.generate(world, rand, i, j, k))
		{
			world.setBlock(i, j, k, this.blockID);
		}
