...
import morethantrees.treegen.WorldGenIceTrees;
...
{
	private Random rand;
	public static final String[] WOOD_TYPES = new String[] {"ice"};
    //private Icon[] blockIcon = new Icon[1];
	
	public BlockIceSapling(int i)
	{
		super(i);
		float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:iceSapling");
    }
	...
	public void generateTree(World world, int i, int j, int k, Random random, int l)
	{
		...
		WorldGenIceTrees icetrees = new WorldGenIceTrees();

		if (!icetrees.generate(world, random, i, j, k))
		{
	...
	public void generate(World world, Random rand, int i, int j, int k)
    {
		world.setBlock(i, j, k, 0);
		WorldGenIceTrees icetrees = new WorldGenIceTrees();
		if (!icetrees.generate(world, rand, i, j, k))
		{
			world.setBlock(i, j, k, this.blockID);
		}
