...
import morethantrees.treegen.WorldGenStoneTrees;
...
{
    private Random               rand;
    public static final String[] WOOD_TYPES = new String[] { "stone" };
    
    public BlockStoneSapling(int i)
    {
        super(i);
        float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:stoneSapling");
    }
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
    {
        ...
        WorldGenStoneTrees stonetrees = new WorldGenStoneTrees();
        
        if (!stonetrees.generate(world, random, i, j, k))
        {
    ...
    public void generate(World world, Random rand, int i, int j, int k)
    {
        world.setBlock(i, j, k, 0);
        WorldGenStoneTrees stonetrees = new WorldGenStoneTrees();
        if (!stonetrees.generate(world, rand, i, j, k))
        {
            world.setBlock(i, j, k, this.blockID);
        }
