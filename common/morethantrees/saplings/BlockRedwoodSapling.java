...
import morethantrees.treegen.WorldGenRedwoodTrees;
...
{
    private Random               rand;
    public static final String[] WOOD_TYPES = new String[] { "redwoodSapling" };
    
    //private Icon[] blockIcon = new Icon[1];
    
    public BlockRedwoodSapling(int i)
    {
        super(i);
        float f = 0.4F;
    ...
    {
        this.blockIcon = var1.registerIcon("MTJT:redwoodSapling");
    }
    ...
    public void generateTree(World world, int i, int j, int k, Random random, int l)
    {
        ...
        WorldGenRedwoodTrees redwoodtrees = new WorldGenRedwoodTrees();
        
        if (!redwoodtrees.generate(world, random, i, j, k))
        {
    ...
    public void generate(World world, Random rand, int i, int j, int k)
    {
        world.setBlock(i, j, k, 0);
        WorldGenRedwoodTrees redwoodtrees = new WorldGenRedwoodTrees();
        if (!redwoodtrees.generate(world, rand, i, j, k))
        {
            world.setBlock(i, j, k, this.blockID);
        }
